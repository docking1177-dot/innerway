...
(function (global) {
  ...
  function openCodeModal(cat, variant, ctx) {
    ...
    const demoBtn = IWCFG.selfServiceCode ? $('#codeDemo', mask) : null;
  ...
  function openPrivacyModal() {
    ...
    mask.innerHTML =
      ...
      '      <li>' + (SRV
        ? '你生成的 AI 解读（含「完整人格档案」聚合解读）<b>会保存在本站服务器</b>：一是便于你再次打开报告时直接展示，二是在你寻求协助时用于核实问题。该内容仅按设备归属访问，不对外提供、不用于其他用途。'
        : '你生成的 AI 解读仅保存在本机浏览器（localStorage），清除浏览器数据后即不可恢复。') + '</li>' +
      '      <li>服务器仅保存专属码、作答、报告与 AI 解读文本，<b>不含 IP、姓名、手机号</b>等身份信息；如需删除本人数据，可通过购买渠道联系我们处理。</li>' +
      '    </ul>' +
  ...
  function boot() {
    ...
    if (demoConsoleBtn) {
      if (IWCFG.adminConsole) demoConsoleBtn.addEventListener('click', openDemoConsole);
