...
(function (global) {
  ...
  // 注：「维度得分拆解」暂时关闭（不再输出版块级维度计分）。
  var UNI_SECTIONS = ['作答风格画像', '立体画像描述', '核心建议', '说明与免责'];
  // 门类专属追加小节：MBTI 原静态报告的「长处 / 盲区 / 职业 / 成长」下沉给 AI 解读承载
  var CAT_EXTRA_SECTIONS = {
    mbti: ['潜在长处', '可能的盲区', '职业方向参考', '成长提示与使用建议']
  };
  // 按门类拼装完整小节序列（追加小节插在「立体画像描述」之后、「核心建议」之前）
  function sectionsFor(catId) {
    var extra = (catId && CAT_EXTRA_SECTIONS[catId]) || [];
    if (!extra.length) return UNI_SECTIONS.slice();
    var out = [];
    UNI_SECTIONS.forEach(function (t) {
      if (t === '核心建议') extra.forEach(function (x) { out.push(x); });
      out.push(t);
    });
    return out;
  }
  ...
  function buildQuizPayload(opts) {
    ...
    return {
      ...
      format: sectionsFor(cat.id || rec.categoryId),
      risk: riskHit !== null,
      riskNo: riskHit
    };
  }
  ...
  function quizMessages(payload) {
    ...
    var isMbti = payload.categoryId === 'mbti';
    var sections = sectionsFor(payload.categoryId);
    var headings = sections.map(function (t) { return '## ' + t; });
    ...
    var steps = [];
    steps.push('正文第一段直接给出【一句话总览】：用 1–2 句话概括本次测评最核心的画像，不使用专业术语，让用户立刻有“被看见”的感觉；不寒暄，不以“以下是/好的”等开头；');
    steps.push('其后严格按下列标题顺序输出；每个标题独占一行、以“## ”开头，标题文字不得增删改：\n' + headings.join('\n'));
    steps.push('「作答风格画像」：分析作答模式（倾向极端或居中、前后一致性、可能的掩饰/惯性倾向、对认真程度的观察等），把数据翻译成有温度的行为模式和感受描述——例如写成“你大多时候选择温和的中间立场，不轻易把话说满，也很少完全否定什么”，而不出现任何档位计数、题量统计或“第几档几次”式的句子；数字只作隐含支撑、不直接罗列；当提到具体题目时，只用题干关键词指代这道题（例如“你在睡眠困难那一题选了几乎每天”），说明该题的选择如何呼应了整体模式，禁止出现任何题号；全篇避免使用“从数据来看”“统计显示”“分析表明”等审视性开头，改用“翻完你的作答”“读下来最明显的是”“你可能常常会发现”等陪伴式表达；');
    steps.push('「立体画像描述」：用 200–300 字把数据还原成一个具体、立体的人；必须同时包含优势面与困扰面，不偏废；可用 1 个贴切的比喻（至多 1 个）；禁止“你就是一个 XX 的人”式贴标签，改用“你可能常常感到…”“在……场景下，你倾向于……”等描述性语言；');
    if (isMbti) {
      steps.push('「潜在长处」：结合本次类型代码与四个维度的偏好方向，列出 3–4 条在合适情境下会成为优势的特点；用描述性语言写清“在什么场景下、以什么方式体现”，不吹捧、不堆砌形容词；');
      steps.push('「可能的盲区」：列出 2–4 条同一份偏好可能带来的代价或容易忽略之处；语气中性、不指责，写清“在什么情况下容易发生、可以怎样留意”，避免制造焦虑；');
      steps.push('「职业方向参考」：基于偏好给出 3–5 个方向性的工作场景或领域举例（不做具体岗位断言），并说明“为什么这些场景与你的偏好相合”；必须以一句提醒收尾——职业选择需结合个人兴趣、能力与价值观综合判断，此处仅为方向参考，不代表“你只能做/适合做某职业”；');
      steps.push('「成长提示与使用建议」：给出 2–3 条把这份类型当作“自我认知工具”使用的建议（如在不同情境下如何借力自己的偏好、如何与偏好相反的人协作、如何避免把类型当成借口）；语气温和、可操作；');
    }
    steps.push('「核心建议」是本篇最轻量的部分：给 1–3 条“可以试试的小方向”即可（没有把握时可只给 1 条或省略），每条以“### 🎯 一句话标题”开头，正文只写一两句可立即执行的简短做法，不展开长段的适用场景/预期效果；建议部分合计不超过全文的约六分之一，不要把解读写成“建议清单”；');
    steps.push('「说明与免责」：说明本解读基于本次自我作答，反映测评指定时间范围内的状态，非终身判定；' + (isMental || risk
      ? '并强调：如为心理健康类内容，症状持续或加重时请寻求精神科/心理科专业人员评估，紧急情况拨打全国心理援助热线 12356 或 120。'
      : '并说明人格/特质类内容无好坏之分，关键在于如何理解并运用自身特点。'));
    steps.push('篇幅与深度：每个小节都要写足（一般 2–4 句或 2–4 条），不要把任何一节用一两句话带过；整体接近但不超过 1500 字，宁可把话说透，也不要草草收尾；');
    steps.push('排版：关键分数、等级与结论一律用 **加粗**；列表项以“- ”开头并单独成行；段落尽量短、多用分块与列表，移动端友好；');
    steps.push('页面会自动渲染“## / ###”标题与加粗，但不会渲染“> 引用块”与“---”分割线：请勿输出这两种语法；「核心建议」下的每条建议请用“### 🎯 标题”起头，页面会把它渲染为同一张卡片内的小标题；');
    steps.push('整体信息密度优先、避免冗余铺垫；把主要篇幅用于帮用户“看懂自己”（作答风格、立体画像' + (isMbti ? '、潜在长处与盲区' : '') + '），建议只作收尾点缀；结尾不需要任何寒暄或署名。');
    var numberedSteps = steps.map(function (s, i) { return (i + 1) + ') ' + s; }).join('\n') + '\n';
  ...
  function buildTatPayload(opts) {
    ...
    return {
      ...
      stories: stories,
      profile: (opts.profile || rec.profile || null),
  ...
  function tatMessages(payload) {
    var sys = '你是一位温柔、细腻、有同理心的心理测评分析师，同时也是一位专注的倾听者。你具备主题统觉测验（Thematic Apperception Test, TAT）的专业解读能力，能够通过来访者对模糊图片所编写的故事，温和地引导其探索内在的动机、情感模式、关系议题和心理冲突。'
      ...
      + ((payload.profile && (payload.profile.gender || payload.profile.age))
        ? '\n\n背景说明：这位来访者自愿填写了基础信息（' + [payload.profile.gender, payload.profile.age].filter(Boolean).join(' · ') + '）。请把它当作轻量的背景参考，用于让措辞更贴合（如不必刻意回避性别与人称），但不要据此对人格、动机或任何故事内容做武断的归因或贴标签——解读仍只以故事本身为依据。'
        : '')
